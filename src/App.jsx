...
const ROOM_ILLUSTRATIONS = {
  ...
  deckfence: (
    <svg viewBox="0 0 140 140" fill="none">
      <line x1="18" y1="76" x2="122" y2="76" stroke="white" strokeWidth="2" />
      <line x1="18" y1="88" x2="122" y2="88" stroke="white" strokeWidth="2" />
      <line x1="18" y1="100" x2="122" y2="100" stroke="white" strokeWidth="2" />
      <line x1="24" y1="56" x2="24" y2="76" stroke="white" strokeWidth="2" />
      <line x1="24" y1="56" x2="116" y2="56" stroke="white" strokeWidth="2" />
      <line x1="70" y1="56" x2="70" y2="76" stroke="white" strokeWidth="1.5" />
      <line x1="116" y1="56" x2="116" y2="76" stroke="white" strokeWidth="2" />
      <line x1="30" y1="26" x2="30" y2="56" stroke="white" strokeWidth="2" />
      <line x1="54" y1="20" x2="54" y2="56" stroke="white" strokeWidth="2" />
      <line x1="78" y1="26" x2="78" y2="56" stroke="white" strokeWidth="2" />
      <line x1="102" y1="20" x2="102" y2="56" stroke="white" strokeWidth="2" />
      <line x1="26" y1="34" x2="106" y2="34" stroke="white" strokeWidth="1.5" />
    </svg>
  ),
  interlocking: (
    <svg viewBox="0 0 140 140" fill="none">
  ...
  epoxyfloor: (
    <svg viewBox="0 0 140 140" fill="none">
      <rect x="18" y="18" width="104" height="104" rx="3" stroke="white" strokeWidth="2" />
      <line x1="18" y1="70" x2="122" y2="70" stroke="white" strokeWidth="1" strokeDasharray="4 4" />
      <line x1="70" y1="18" x2="70" y2="122" stroke="white" strokeWidth="1" strokeDasharray="4 4" />
      <path d="M30 42 Q42 30 56 42 T84 42" stroke="white" strokeWidth="1.5" />
      <circle cx="98" cy="96" r="3" fill="white" />
      <circle cx="108" cy="104" r="2" fill="white" />
      <circle cx="90" cy="106" r="1.6" fill="white" />
    </svg>
  ),
  glass: (
    <svg viewBox="0 0 140 140" fill="none">
...
function estimateTimeLabel(roomTypeId) {
  const count = (QUESTIONS[roomTypeId] || []).length;
  const seconds = count * 7; // rough average per question, including simple taps and number entry
  if (seconds < 60) return `~${Math.round(seconds / 5) * 5 || 5} sec`;
...
const QUESTIONS = {
  kitchen: [
    ...
    { id: "kitchenDemoNeeded", label: "Does the kitchen need demolition first (old cabinets/counters removed)?", type: "select", options: ["Yes", "No"] },
    { id: "cabinetType", label: "Stock/ready-made cabinets, custom-made, or not needed?", type: "select", options: ["Stock/ready-made", "Custom-made", "Not needed"] },
    ...
    { id: "backsplash", label: "Backsplash — tile, quartz slab, or none?", type: "select", options: ["Tile", "Quartz", "None"] },
  ...
  bathroom: [
    ...
    { id: "bathDemoNeeded", label: "Does the bathroom need demolition first (old fixtures/tile out)?", type: "select", options: ["Yes", "No"] },
    ...
    { id: "tubShower", label: "Tub, shower, both, or none?", type: "select", options: ["Tub", "Shower", "Both", "None"] },
    {
      id: "showerBaseType",
  ...
  basement: [
    { id: "layoutMethod", label: "How do you want to enter the size?", type: "select", options: ["Simple rectangle (one open space)", "Room by room (exact, most accurate)", "Approximate (I know room count + overall size, not exact rooms)"] },
    { id: "approxRoomCount", label: "How many rooms/partitions, roughly?", type: "number", unit: "rooms", min: 1, max: 8, step: 1, default: 3, visibleIf: (a) => a.layoutMethod === "Approximate (I know room count + overall size, not exact rooms)" },
    { id: "approxSizeKnown", label: "Do you know the building's length × width, or just the total square footage?", type: "select", options: ["Length × width", "Just total sq ft"], visibleIf: (a) => a.layoutMethod === "Approximate (I know room count + overall size, not exact rooms)" },
    { id: "approxLength", label: "Overall length?", type: "number", unit: "ft", min: 10, max: 80, step: 1, default: 35, visibleIf: (a) => a.layoutMethod === "Approximate (I know room count + overall size, not exact rooms)" && a.approxSizeKnown !== "Just total sq ft" },
    { id: "approxWidth", label: "Overall width?", type: "number", unit: "ft", min: 10, max: 80, step: 1, default: 25, visibleIf: (a) => a.layoutMethod === "Approximate (I know room count + overall size, not exact rooms)" && a.approxSizeKnown !== "Just total sq ft" },
    { id: "approxSqft", label: "Total square footage?", type: "number", unit: "sq ft", min: 200, max: 4000, step: 10, default: 900, visibleIf: (a) => a.layoutMethod === "Approximate (I know room count + overall size, not exact rooms)" && a.approxSizeKnown === "Just total sq ft" },
    { id: "length", label: "Basement length?", type: "number", unit: "ft", min: 10, max: 60, step: 1, default: 30, visibleIf: (a) => a.layoutMethod !== "Room by room (more accurate perimeter)" },
    { id: "width", label: "Basement width?", type: "number", unit: "ft", min: 10, max: 50, step: 1, default: 20, visibleIf: (a) => a.layoutMethod !== "Room by room (more accurate perimeter)" },
    { id: "basementRoomCount", label: "How many separate rooms/spaces? (up to 10)", type: "number", unit: "rooms", min: 1, max: 10, step: 1, default: 3, visibleIf: (a) => a.layoutMethod === "Room by room (exact, most accurate)" },
    { id: "bRoom1Length", label: "Room 1 — length?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 11, visibleIf: (a) => a.layoutMethod === "Room by room (exact, most accurate)" },
    { id: "bRoom1Width", label: "Room 1 — width?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 13, visibleIf: (a) => a.layoutMethod === "Room by room (exact, most accurate)" },
    { id: "bRoom2Length", label: "Room 2 — length?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 11, visibleIf: (a) => a.layoutMethod === "Room by room (exact, most accurate)" && (a.basementRoomCount ?? 3) >= 2 },
    { id: "bRoom2Width", label: "Room 2 — width?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 13, visibleIf: (a) => a.layoutMethod === "Room by room (exact, most accurate)" && (a.basementRoomCount ?? 3) >= 2 },
    { id: "bRoom3Length", label: "Room 3 — length?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 11, visibleIf: (a) => a.layoutMethod === "Room by room (exact, most accurate)" && (a.basementRoomCount ?? 3) >= 3 },
    { id: "bRoom3Width", label: "Room 3 — width?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 13, visibleIf: (a) => a.layoutMethod === "Room by room (exact, most accurate)" && (a.basementRoomCount ?? 3) >= 3 },
    { id: "bRoom4Length", label: "Room 4 — length?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 10, visibleIf: (a) => a.layoutMethod === "Room by room (exact, most accurate)" && (a.basementRoomCount ?? 3) >= 4 },
    { id: "bRoom4Width", label: "Room 4 — width?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 12, visibleIf: (a) => a.layoutMethod === "Room by room (exact, most accurate)" && (a.basementRoomCount ?? 3) >= 4 },
    { id: "bRoom5Length", label: "Room 5 — length?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 6, visibleIf: (a) => a.layoutMethod === "Room by room (exact, most accurate)" && (a.basementRoomCount ?? 3) >= 5 },
    { id: "bRoom5Width", label: "Room 5 — width?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 8, visibleIf: (a) => a.layoutMethod === "Room by room (exact, most accurate)" && (a.basementRoomCount ?? 3) >= 5 },
    { id: "bRoom6Length", label: "Room 6 — length?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 10, visibleIf: (a) => a.layoutMethod === "Room by room (exact, most accurate)" && (a.basementRoomCount ?? 3) >= 6 },
    { id: "bRoom6Width", label: "Room 6 — width?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 10, visibleIf: (a) => a.layoutMethod === "Room by room (exact, most accurate)" && (a.basementRoomCount ?? 3) >= 6 },
    { id: "bRoom7Length", label: "Room 7 — length?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 10, visibleIf: (a) => a.layoutMethod === "Room by room (exact, most accurate)" && (a.basementRoomCount ?? 3) >= 7 },
    { id: "bRoom7Width", label: "Room 7 — width?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 10, visibleIf: (a) => a.layoutMethod === "Room by room (exact, most accurate)" && (a.basementRoomCount ?? 3) >= 7 },
    { id: "bRoom8Length", label: "Room 8 — length?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 10, visibleIf: (a) => a.layoutMethod === "Room by room (exact, most accurate)" && (a.basementRoomCount ?? 3) >= 8 },
    { id: "bRoom8Width", label: "Room 8 — width?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 10, visibleIf: (a) => a.layoutMethod === "Room by room (exact, most accurate)" && (a.basementRoomCount ?? 3) >= 8 },
    { id: "bRoom9Length", label: "Room 9 — length?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 10, visibleIf: (a) => a.layoutMethod === "Room by room (exact, most accurate)" && (a.basementRoomCount ?? 3) >= 9 },
    { id: "bRoom9Width", label: "Room 9 — width?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 10, visibleIf: (a) => a.layoutMethod === "Room by room (exact, most accurate)" && (a.basementRoomCount ?? 3) >= 9 },
    { id: "bRoom10Length", label: "Room 10 — length?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 10, visibleIf: (a) => a.layoutMethod === "Room by room (exact, most accurate)" && (a.basementRoomCount ?? 3) >= 10 },
    { id: "bRoom10Width", label: "Room 10 — width?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 10, visibleIf: (a) => a.layoutMethod === "Room by room (exact, most accurate)" && (a.basementRoomCount ?? 3) >= 10 },
    ...
    { id: "bathroomCount", label: "How many bathrooms are you adding down there?", type: "number", unit: "bathrooms", min: 0, max: 2, step: 1, default: 0 },
  ...
  deckfence: [
    { id: "dfScope", label: "What do you need?", type: "select", options: ["Fence", "Deck"] },
    { id: "length", label: "How many linear feet of fence?", type: "number", unit: "linear ft", min: 10, max: 500, step: 5, default: 100, visibleIf: (a) => a.dfScope === "Fence" },
    { id: "height", label: "Fence height?", type: "select", options: ["6 ft (standard)", "8 ft (tall)"], visibleIf: (a) => a.dfScope === "Fence" },
    { id: "gateCount", label: "How many gates?", type: "number", unit: "gates", min: 0, max: 5, step: 1, default: 0, visibleIf: (a) => a.dfScope === "Fence" },
    { id: "fenceRepairNeeded", label: "Also a small repair somewhere else along the fence (a few boards/posts)?", type: "select", options: ["Yes", "No"], visibleIf: (a) => a.dfScope === "Fence" },
    { id: "sqft", label: "Deck size?", type: "number", unit: "sq ft", min: 50, max: 1000, step: 10, default: 200, visibleIf: (a) => a.dfScope === "Deck" },
    { id: "material", label: "Decking material?", type: "select", options: ["Pressure-treated", "Composite"], visibleIf: (a) => a.dfScope === "Deck" },
    { id: "railingLength", label: "How many linear feet of railing?", type: "number", unit: "linear ft", min: 0, max: 200, step: 5, default: 40, visibleIf: (a) => a.dfScope === "Deck" },
    { id: "railingMaterial", label: "Railing material?", type: "select", options: ["Wood", "Aluminum"], visibleIf: (a) => a.dfScope === "Deck" && (a.railingLength ?? 0) > 0 },
  ],
  interlocking: [
    ...
    {
      id: "ilMaterial",
      label: "Paver, poured concrete, or outdoor porcelain tile?",
      type: "select",
      options: ["Paver", "Poured concrete (walkway/steps)", "Outdoor porcelain tile"],
    },
    { id: "tileSource", label: "Use our standard paver, or are you supplying your own?", type: "select", options: ["Use standard paver (included)", "I'll supply my own paver"], visibleIf: (a) => a.ilMaterial === "Paver" || !a.ilMaterial },
    { id: "stepsCount", label: "How many concrete steps (if any)?", type: "number", unit: "steps", min: 0, max: 15, step: 1, default: 0, visibleIf: (a) => a.ilMaterial === "Poured concrete (walkway/steps)" },
    { id: "stepsTile", label: "Tile over the concrete steps?", type: "select", options: ["Yes", "No"], visibleIf: (a) => a.ilMaterial === "Poured concrete (walkway/steps)" && (a.stepsCount ?? 0) > 0 },
    { id: "oldConcreteRemoval", label: "Does old concrete/asphalt need to come out first?", type: "select", options: ["Yes", "No"], visibleIf: (a) => a.ilMaterial === "Poured concrete (walkway/steps)" },
  ],
  glass: [
  ...
  flood: [
    ...
    { id: "floodLayoutMethod", label: "How do you want to enter the size?", type: "select", options: ["Room by room (exact, most accurate)", "Approximate (I know room count + overall size, not exact rooms)"] },
    { id: "roomCount", label: "How many rooms/spaces were flooded? (up to 10)", type: "number", unit: "rooms", min: 1, max: 10, step: 1, default: 1, visibleIf: (a) => a.floodLayoutMethod !== "Approximate (I know room count + overall size, not exact rooms)" },
    { id: "room1Length", label: "Room 1 — length?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 20, visibleIf: (a) => a.floodLayoutMethod !== "Approximate (I know room count + overall size, not exact rooms)" },
    { id: "room1Width", label: "Room 1 — width?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 20, visibleIf: (a) => a.floodLayoutMethod !== "Approximate (I know room count + overall size, not exact rooms)" },
    { id: "room2Length", label: "Room 2 — length?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 12, visibleIf: (a) => a.floodLayoutMethod !== "Approximate (I know room count + overall size, not exact rooms)" && (a.roomCount ?? 1) >= 2 },
    { id: "room2Width", label: "Room 2 — width?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 10, visibleIf: (a) => a.floodLayoutMethod !== "Approximate (I know room count + overall size, not exact rooms)" && (a.roomCount ?? 1) >= 2 },
    { id: "room3Length", label: "Room 3 — length?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 12, visibleIf: (a) => a.floodLayoutMethod !== "Approximate (I know room count + overall size, not exact rooms)" && (a.roomCount ?? 1) >= 3 },
    { id: "room3Width", label: "Room 3 — width?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 10, visibleIf: (a) => a.floodLayoutMethod !== "Approximate (I know room count + overall size, not exact rooms)" && (a.roomCount ?? 1) >= 3 },
    { id: "room4Length", label: "Room 4 — length?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 12, visibleIf: (a) => a.floodLayoutMethod !== "Approximate (I know room count + overall size, not exact rooms)" && (a.roomCount ?? 1) >= 4 },
    { id: "room4Width", label: "Room 4 — width?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 10, visibleIf: (a) => a.floodLayoutMethod !== "Approximate (I know room count + overall size, not exact rooms)" && (a.roomCount ?? 1) >= 4 },
    { id: "room5Length", label: "Room 5 — length?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 12, visibleIf: (a) => a.floodLayoutMethod !== "Approximate (I know room count + overall size, not exact rooms)" && (a.roomCount ?? 1) >= 5 },
    { id: "room5Width", label: "Room 5 — width?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 10, visibleIf: (a) => a.floodLayoutMethod !== "Approximate (I know room count + overall size, not exact rooms)" && (a.roomCount ?? 1) >= 5 },
    { id: "room6Length", label: "Room 6 — length?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 12, visibleIf: (a) => a.floodLayoutMethod !== "Approximate (I know room count + overall size, not exact rooms)" && (a.roomCount ?? 1) >= 6 },
    { id: "room6Width", label: "Room 6 — width?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 10, visibleIf: (a) => a.floodLayoutMethod !== "Approximate (I know room count + overall size, not exact rooms)" && (a.roomCount ?? 1) >= 6 },
    { id: "room7Length", label: "Room 7 — length?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 12, visibleIf: (a) => a.floodLayoutMethod !== "Approximate (I know room count + overall size, not exact rooms)" && (a.roomCount ?? 1) >= 7 },
    { id: "room7Width", label: "Room 7 — width?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 10, visibleIf: (a) => a.floodLayoutMethod !== "Approximate (I know room count + overall size, not exact rooms)" && (a.roomCount ?? 1) >= 7 },
    { id: "room8Length", label: "Room 8 — length?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 12, visibleIf: (a) => a.floodLayoutMethod !== "Approximate (I know room count + overall size, not exact rooms)" && (a.roomCount ?? 1) >= 8 },
    { id: "room8Width", label: "Room 8 — width?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 10, visibleIf: (a) => a.floodLayoutMethod !== "Approximate (I know room count + overall size, not exact rooms)" && (a.roomCount ?? 1) >= 8 },
    { id: "room9Length", label: "Room 9 — length?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 12, visibleIf: (a) => a.floodLayoutMethod !== "Approximate (I know room count + overall size, not exact rooms)" && (a.roomCount ?? 1) >= 9 },
    { id: "room9Width", label: "Room 9 — width?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 10, visibleIf: (a) => a.floodLayoutMethod !== "Approximate (I know room count + overall size, not exact rooms)" && (a.roomCount ?? 1) >= 9 },
    { id: "room10Length", label: "Room 10 — length?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 12, visibleIf: (a) => a.floodLayoutMethod !== "Approximate (I know room count + overall size, not exact rooms)" && (a.roomCount ?? 1) >= 10 },
    { id: "room10Width", label: "Room 10 — width?", type: "number", unit: "ft", min: 3, max: 40, step: 1, default: 10, visibleIf: (a) => a.floodLayoutMethod !== "Approximate (I know room count + overall size, not exact rooms)" && (a.roomCount ?? 1) >= 10 },
    { id: "floodApproxRoomCount", label: "How many rooms/partitions, roughly?", type: "number", unit: "rooms", min: 1, max: 8, step: 1, default: 3, visibleIf: (a) => a.floodLayoutMethod === "Approximate (I know room count + overall size, not exact rooms)" },
    { id: "floodApproxSizeKnown", label: "Do you know the building's length × width, or just the total square footage?", type: "select", options: ["Length × width", "Just total sq ft"], visibleIf: (a) => a.floodLayoutMethod === "Approximate (I know room count + overall size, not exact rooms)" },
    { id: "floodApproxLength", label: "Overall length?", type: "number", unit: "ft", min: 10, max: 80, step: 1, default: 35, visibleIf: (a) => a.floodLayoutMethod === "Approximate (I know room count + overall size, not exact rooms)" && a.floodApproxSizeKnown !== "Just total sq ft" },
    { id: "floodApproxWidth", label: "Overall width?", type: "number", unit: "ft", min: 10, max: 80, step: 1, default: 25, visibleIf: (a) => a.floodLayoutMethod === "Approximate (I know room count + overall size, not exact rooms)" && a.floodApproxSizeKnown !== "Just total sq ft" },
    { id: "floodApproxSqft", label: "Total square footage?", type: "number", unit: "sq ft", min: 200, max: 4000, step: 10, default: 900, visibleIf: (a) => a.floodLayoutMethod === "Approximate (I know room count + overall size, not exact rooms)" && a.floodApproxSizeKnown === "Just total sq ft" },
    ...
    { id: "floodWallDrywallThickness", label: "Wall drywall — 1/2\" (standard) or 5/8\" (fire-rated)?", type: "select", options: ["1/2\" (standard)", "5/8\" (fire-rated)"] },
    { id: "floodHasCeiling", label: "Is there a ceiling here that needs drywall too?", type: "select", options: ["Yes", "No"] },
    { id: "floodCeilingDrywallThickness", label: "Ceiling drywall — 1/2\" (standard) or 5/8\" (fire-rated)?", type: "select", options: ["1/2\" (standard)", "5/8\" (fire-rated)"], visibleIf: (a) => a.floodHasCeiling === "Yes" },
    ...
    { id: "floodFlooringNeeded", label: "Does the floor need new flooring installed (as part of the rebuild)?", type: "select", options: ["Yes — vinyl (LVP)", "Yes — tile", "Yes — epoxy (basic)", "No — not part of this scope"] },
    ...
    { id: "contentManipulation", label: "Do you need to move a lot of furniture/belongings to access the work area?", type: "select", options: ["Yes", "No"] },
  ...
  epoxyfloor: [
    { id: "efLength", label: "Space length?", type: "number", unit: "ft", min: 5, max: 60, step: 1, default: 20 },
    { id: "efWidth", label: "Space width?", type: "number", unit: "ft", min: 5, max: 60, step: 1, default: 20 },
    { id: "efSpaceType", label: "Basement or garage?", type: "select", options: ["Basement", "Garage"] },
    {
      id: "efTier",
      label: "Finish level?",
      type: "select",
      options: [
        "Basic (grind, crack repair, base coat, flake, topcoat)",
        "Premium (concrete repair, metallic/premium flake, multi-coat)",
        "Decorative/metallic (full custom colour, showroom finish)",
      ],
    },
    { id: "efMoistureElevated", label: "Is there elevated moisture in the concrete? (moisture test is part of standard prep either way)", type: "select", options: ["Yes — needs vapour-barrier primer", "No / not sure yet"] },
    { id: "efCoatingRemoval", label: "Any old paint or coating on the concrete that needs removing first?", type: "select", options: ["Yes", "No"] },
    { id: "efAdhesiveRemoval", label: "Any old glue/adhesive residue (from old tile or carpet) to remove first?", type: "select", options: ["Yes", "No"] },
    { id: "efSelfLevel", label: "Does the floor need self-leveling compound first (uneven slab)?", type: "select", options: ["Yes", "No"] },
    {
      id: "efCrackSeverity",
      label: "Any cracks in the concrete?",
      type: "select",
      options: ["None / minor (included in standard prep)", "Moderate — needs routing/sealing", "Major or structural — needs inspection", "Active water intrusion"],
    },
    {
      id: "efCrackLinearFt",
      label: "Moderate cracks — total linear feet?",
      type: "number",
      unit: "linear ft",
      min: 1,
      max: 100,
      step: 1,
      default: 10,
      visibleIf: (a) => a.efCrackSeverity === "Moderate — needs routing/sealing",
    },
  ],
};

// Reuses a full question set from another room (unchanged, same visibleIf
// logic) inside a different flow, by prefixing every question id and
// rewriting visibleIf to read from a de-prefixed view of the answers object.
// This is how Flood Restoration pulls in the complete Bathroom flow when the
// person says there's a bathroom involved too — no duplicated questions to
// maintain separately.
function prefixQuestions(questions, prefix, gateVisibleIf, extraView) {
  return questions.map((q) => ({
    ...q,
    id: `${prefix}${q.id}`,
    visibleIf: (a) => {
      if (gateVisibleIf && !gateVisibleIf(a)) return false;
      if (!q.visibleIf) return true;
      const view = {};
      Object.keys(a).forEach((k) => {
        if (k.startsWith(prefix)) view[k.slice(prefix.length)] = a[k];
      });
      if (extraView) Object.assign(view, extraView(a));
      return q.visibleIf(view);
    },
  }));
}

QUESTIONS.flood = QUESTIONS.flood.concat(
  prefixQuestions(
    QUESTIONS.bathroom,
    "floodBath_",
    (a) => a.floodHasBathroom === "Yes"
  )
);

QUESTIONS.basement = QUESTIONS.basement.concat(
  prefixQuestions(
    QUESTIONS.bathroom.filter((q) => q.id !== "scope"),
    "bBath1_",
    (a) => (a.bathroomCount ?? 0) >= 1,
    () => ({ scope: "Full renovation" })
  ),
  prefixQuestions(
    QUESTIONS.bathroom.filter((q) => q.id !== "scope"),
    "bBath2_",
    (a) => (a.bathroomCount ?? 0) >= 2,
    () => ({ scope: "Full renovation" })
  )
);
...
const RATE_META = {
  kitchen: [
    ...
    { key: "backsplashQuartzPerSqft", label: "Backsplash — quartz slab", unit: "$/sq ft", value: 65 },
    ...
    { key: "electricalPointEach", label: "Outlet/light point", unit: "$/each", value: 50 },
    { key: "plumbingRelocationFlat", label: "Plumbing relocation (moving sink/stove location)", unit: "$ flat", value: 450 },
    { key: "paintPerSqft", label: "Paint (walls + ceiling, per floor sq ft)", unit: "$/sq ft", value: 3.6 },
  ],
  bathroom: [
    ...
    { key: "floorProtectionPerSqft", label: "Floor protection (Ram Board, job site)", unit: "$/sq ft", value: 0.5 },
    ...
    { key: "underlaymentStandardPerSqft", label: "Underlayment, standard (main floor, non-heated — cement board/uncoupling membrane)", unit: "$/sq ft", value: 2 },
    ...
    { key: "nicheReadyMadeFlat", label: "Niche, ready-made — with installation", unit: "$/each", value: 220 },
    { key: "nicheCustomFlat", label: "Niche, custom-built — starting price", unit: "$/each", value: 350 },
    ...
    { key: "waterproofingFullMembranePerSqft", label: "Waterproofing, full membrane", unit: "$/sq ft", value: 3.25 },
    ...
    { key: "showerJambFlat", label: "Shower jamb/threshold (closes off a standard curb)", unit: "$/each", value: 145 },
  ...
  basement: [
    ...
    { key: "wallInsulationPerSqft", label: "Wall insulation (vapour barrier + batt + poly)", unit: "$/sq ft", value: 3.2 },
    ...
    { key: "drywallInstallOnlyPerSheet", label: "Drywall Installation — hang only, no taping (per sheet, 4×8 = 32 sq ft)", unit: "$/each", value: 80 },
    { key: "popcornCeilingRemovalPerSqft", label: "Popcorn ceiling removal (scrape, patch dots/spots, no asbestos)", unit: "$/sq ft", value: 3 },
    { key: "ceilingRetexturePerSqft", label: "Full skim coat/recoat (beyond spot patching)", unit: "$/sq ft", value: 1.5 },
    { key: "drywallHalfInchPerSqft", label: "Drywall + taping (walls), 1/2\" standard, ready for paint", unit: "$/sq ft", value: 3.25 },
    ...
    { key: "potLightEach", label: "Pot light, individually (with wiring)", unit: "$/each", value: 60 },
    ...
    { key: "doorTrimOnlyEach", label: "Door trim/casing only, no door replacement", unit: "$/each", value: 130 },
    { key: "doorOpeningWideningFlat", label: "Door opening widening", unit: "$/each", value: 380 },
    { key: "caulkingFlat", label: "Caulking (doors/trim, whole job)", unit: "$ flat", value: 250 },
    { key: "glassRailingsFlat", label: "Glass railings, incl. hardware", unit: "$ flat", value: 1300 },
    { key: "paintPrepPerSqft", label: "Paint prep — scrape/patch/prime (wall area basis)", unit: "$/sq ft", value: 0.4 },
    { key: "outletReplaceEach", label: "Outlet, replace existing (same location, no new wiring)", unit: "$/each", value: 22 },
    { key: "switchReplaceEach", label: "Switch, replace existing (same location, no new wiring)", unit: "$/each", value: 22 },
    { key: "gfciReplaceEach", label: "GFCI outlet, replace existing", unit: "$/each", value: 32 },
    ...
    { key: "paintPerSqft", label: "Paint (walls + ceiling, per wall+floor sq ft)", unit: "$/sq ft", value: 2 },
    ...
    { key: "floorTileBasementPerSqft", label: "Floor — tile (concrete slab, no underlayment)", unit: "$/sq ft", value: 8.5 },
    { key: "floorEpoxyStandardPerSqft", label: "Floor — epoxy coating, standard (grind, crack repair, base coat, flake, topcoat)", unit: "$/sq ft", value: 8 },
    { key: "floorEpoxyPremiumPerSqft", label: "Floor — epoxy coating, premium (concrete repair, metallic/premium flake, multi-coat, custom colour)", unit: "$/sq ft", value: 12 },
    { key: "epoxyMoisturePrimerPerSqft", label: "Vapour-barrier primer (elevated moisture)", unit: "$/sq ft", value: 1.5 },
    { key: "concreteGrindingPerSqft", label: "Concrete floor grinding/prep", unit: "$/sq ft", value: 3 },
    { key: "concreteCoatingRemovalPerSqft", label: "Old paint/coating removal from concrete", unit: "$/sq ft", value: 2 },
  ...
  roof: [
    ...
    { key: "sidingRepairPerSqft", label: "Siding repair (patch/section)", unit: "$/sq ft", value: 8 },
    { key: "soffitFasciaRepairPerLinFt", label: "Soffit/fascia repair", unit: "$/linear ft", value: 18 },
    { key: "gutterGuardsPerLinFt", label: "Gutter guards install", unit: "$/linear ft", value: 7 },
  ],
  paint: [
  ...
  deckfence: [
    ...
    { key: "fenceRepairFlat", label: "Fence repair (panel/board section)", unit: "$ flat", value: 600 },
    { key: "fencePostReplaceEach", label: "Fence post replacement, single", unit: "$/each", value: 235 },
    { key: "fenceStainingPerLinFt", label: "Fence staining/sealing", unit: "$/linear ft", value: 8 },
    ...
    { key: "deckBoardRepairPerSqft", label: "Deck board repair/replacement", unit: "$/sq ft", value: 12 },
    { key: "deckStainingPerSqft", label: "Deck staining/sealing", unit: "$/sq ft", value: 2 },
    { key: "deckPostRepairEach", label: "Deck post/support repair, single", unit: "$/each", value: 500 },
  ],
  interlocking: [
  ...
  flood: [
    ...
    { key: "moldCleaningFlat", label: "Mold cleaning treatment (surface, not full remediation)", unit: "$ flat", value: 450 },
    { key: "floodDrywallRebuildPerSqft", label: "New drywall, 1/2\" standard + taping (rebuild the cut band)", unit: "$/sq ft", value: 3.25 },
    { key: "floodDrywallRebuildFireRatedPerSqft", label: "New drywall, 5/8\" fire-rated + taping (rebuild the cut band)", unit: "$/sq ft", value: 3.7 },
    { key: "floodCeilingDrywallPerSqft", label: "Ceiling drywall, 1/2\" standard + taping", unit: "$/sq ft", value: 3.2 },
    { key: "floodCeilingDrywallFireRatedPerSqft", label: "Ceiling drywall, 5/8\" fire-rated + taping", unit: "$/sq ft", value: 3.65 },
    ...
    { key: "floodFlooringEpoxyPerSqft", label: "New flooring, epoxy (basic) — Labour & Supply", unit: "$/sq ft", value: 8 },
    { key: "floodFlooringTilePerSqft", label: "New flooring, tile (concrete slab, no underlayment) — Installation", unit: "$/sq ft", value: 8.5 },
    ...
    { key: "floodFurnitureHaulAwayPerCabinet", label: "Built-in furniture — total loss, haul away", unit: "$/cabinet", value: 20 },
    { key: "floodCabinetDemoPerCabinet", label: "Cabinet demo/removal (before repair)", unit: "$/cabinet", value: 45 },
    { key: "floorProtectionPerSqft", label: "Floor protection (Ram Board, job site)", unit: "$/sq ft", value: 0.5 },
    { key: "contentManipulationFlat", label: "Content manipulation (moving furniture/belongings)", unit: "$ flat", value: 450 },
  ],
  epoxyfloor: [
    { key: "efBasicPerSqft", label: "Epoxy — basic (grind, crack repair, base coat, flake, topcoat)", unit: "$/sq ft", value: 8 },
    { key: "efPremiumPerSqft", label: "Epoxy — premium (concrete repair, metallic/premium flake, multi-coat)", unit: "$/sq ft", value: 12 },
    { key: "efDecorativePerSqft", label: "Epoxy — decorative/metallic (full custom colour, showroom finish)", unit: "$/sq ft", value: 16 },
    { key: "efGaragePremiumFlat", label: "Garage-scale surcharge (vehicle traffic, oil resistance additive)", unit: "$ flat", value: 150 },
    { key: "efMoisturePrimerPerSqft", label: "Vapour-barrier primer (elevated moisture)", unit: "$/sq ft", value: 1.5 },
    { key: "efCoatingRemovalPerSqft", label: "Old paint/coating removal", unit: "$/sq ft", value: 2 },
    { key: "efAdhesiveRemovalPerSqft", label: "Old adhesive/glue removal", unit: "$/sq ft", value: 2 },
    { key: "efSelfLevelPerSqft", label: "Self-leveling compound", unit: "$/sq ft", value: 3.5 },
...
function computeKitchen(a, rates) {
  ...
  if (a.kitchenDemoNeeded === "Yes" && r.demoFlat != null) items.push({ cat: "Demolition & prep", item: "Full demo + haul-away", cost: r.demoFlat, note: "Ballpark for a typical kitchen — confirmed once we see it." });
  ...
  } else if (a.cabinetType === "Not needed") {
    // skip — no cabinet line at all
  ...
  if (a.backsplash === "Tile" && r.backsplashPerSqft != null) {
  ...
  } else if (a.backsplash === "Quartz" && r.backsplashQuartzPerSqft != null) {
    const bsSqft = runLength * 1.5;
    items.push({ cat: "Tile work", item: "Backsplash — quartz slab — Labour & Supply", cost: bsSqft * r.backsplashQuartzPerSqft, note: "Template, fabrication, and install — quartz material included. Approximate — confirm against the fabricator's actual quote." });
  }
  ...
  if (r.sinkHookupFlat != null) items.push({ cat: "Plumbing", item: "Sink plumbing hookup — Labour & Supply", cost: r.sinkHookupFlat, note: "Cut-in and connection either way — new sink or reconnecting the old one.", buys: { name: "Kitchen sink (only if replacing)", low: 250, high: 250 } });
...
function computeBathroom(a, rates) {
  ...
  if (isFullReno) {
    ...
    } else if (BATH.cleanupDisposalFlat != null) {
      items.push({ cat: "Demolition & prep", item: "Cleanup disposal", cost: BATH.cleanupDisposalFlat, note: "No demo needed — just hauling away packaging and offcuts after the work is done." });
    }
  }
  ...
  if (a.floorTileReplace !== "No" && mainFloorSqft > 0) {
    if (a.floorMaterial === "Vinyl (LVP)" && BATH.floorVinylPerSqft != null) {
      ...
      items.push({ cat: "Flooring", item: `Floor tile (${a.tileType || "Standard tile"}) — Installation`, cost: mainFloorSqft * floorTileRate, note: "Install with thinset and grout included — tile and underlayment priced separately. Excludes the shower footprint, priced separately above.", buys: { name: "Floor tile", low: Math.round(mainFloorSqft * 3), high: Math.round(mainFloorSqft * 4) }, materialQty: { type: "Tile — floor", unit: "sq ft", qty: mainFloorSqft } });
      if (a.heatedFloor !== "Yes" && BATH.underlaymentStandardPerSqft != null) {
        items.push({ cat: "Flooring", item: "Underlayment, standard (cement board/uncoupling membrane) — Labour & Supply", cost: mainFloorSqft * BATH.underlaymentStandardPerSqft, note: "Main floor, not a concrete slab — tile always needs an underlayment layer here." });
      }
    }
  }
  ...
  if (a.showerNiche === "Yes" && !hasAcrylicBase) {
    ...
    if (nicheRate != null) {
      items.push({ cat: "Tile work", item: `Shower niche (${a.nicheType || "Ready-made"}) — Installation`, cost: nicheRate, note: isCustomNiche ? "Starting price — built to size, waterproofed and tiled in place. Can run higher for larger or multi-shelf builds." : "Pre-made niche box, waterproofed and finished in place, all-in." });
    }
  }
  ...
  if (BATH.floorProtectionPerSqft != null) {
    items.push({ cat: "Prep", item: "Floor protection", cost: sqft * BATH.floorProtectionPerSqft, note: "Ram Board or equivalent — laid down at the start, protects finished floors for the duration of the job." });
  }
...
function computeBasement(a, rates) {
  ...
  const r = rateMap(list);
  let sqft, perimeter;
  if (a.layoutMethod === "Room by room (exact, most accurate)") {
    const roomCount = a.basementRoomCount ?? 3;
    const rooms = [];
    for (let i = 1; i <= roomCount; i++) {
      rooms.push({ length: a[`bRoom${i}Length`] ?? 10, width: a[`bRoom${i}Width`] ?? 10 });
    }
    sqft = rooms.reduce((s, rm) => s + rm.length * rm.width, 0);
    perimeter = rooms.reduce((s, rm) => s + 2 * (rm.length + rm.width), 0);
  } else if (a.layoutMethod === "Approximate (I know room count + overall size, not exact rooms)") {
    // Exterior perimeter comes from the building footprint (given directly, or
    // backed out from total sq ft assuming a roughly square footprint). Each
    // partition wall beyond the first room is assumed to run the shorter side
    // of the building (the typical, most efficient way to divide a rectangle),
    // and counts twice since drywall goes on both faces.
    let bLength, bWidth;
    if (a.approxSizeKnown === "Just total sq ft") {
      const approxSqft = a.approxSqft ?? 900;
      const side = Math.sqrt(approxSqft);
      bLength = side;
      bWidth = side;
    } else {
      bLength = a.approxLength ?? 35;
      bWidth = a.approxWidth ?? 25;
    }
    sqft = bLength * bWidth;
    const exteriorPerimeter = 2 * (bLength + bWidth);
    const longerSide = Math.max(bLength, bWidth);
    const shorterSide = Math.min(bLength, bWidth);
    const roomCount = a.approxRoomCount ?? 3;
    // Rooms are assumed laid out as a grid: 1 row for up to 3 rooms (simple
    // side-by-side split), 2 rows once there are 4+ rooms — matching how a
    // basement actually gets divided in practice. Short partitions (running
    // the shorter building dimension) create the columns within a row; one
    // long partition (running the longer dimension) splits the rows. Every
    // wall counts twice since drywall goes on both faces.
    const rows = roomCount >= 4 ? 2 : 1;
    const columns = Math.ceil(roomCount / rows);
    const shortPartitions = Math.max(0, columns - 1);
    const longPartitions = Math.max(0, rows - 1);
    const addedInteriorPerimeter = shortPartitions * shorterSide * 2 + longPartitions * longerSide * 2;
    perimeter = exteriorPerimeter + addedInteriorPerimeter;
  } else {
  ...
  const items = [];
  const flags = [];
  if (a.layoutMethod === "Approximate (I know room count + overall size, not exact rooms)") {
    flags.push(`Perimeter estimated from room count, not exact measurements: ${perimeter.toFixed(0)} linear ft (exterior walls + partitions, each assumed to run the shorter building dimension). Enter exact room dimensions for a firmer number.`);
  }
  ...
  const bathroomFraming = (a.bathroomCount ?? 0) > 0 ? 25 * (a.bathroomCount ?? 0) : 0;
  ...
  const basementBathroomCount = a.bathroomCount ?? 0;
  for (let i = 1; i <= basementBathroomCount; i++) {
    const prefix = `bBath${i}_`;
    const bathAnswers = { scope: "Full renovation" };
    Object.keys(a).forEach((k) => {
      if (k.startsWith(prefix)) bathAnswers[k.slice(prefix.length)] = a[k];
    });
    const bathResult = computeBathroom(bathAnswers, rates);
    bathResult.items.forEach((it) => items.push({ ...it, item: `Basement bathroom #${i} — ${it.item}` }));
  }
  ...
  if (r.baseboardPerLinFt != null) {
    items.push({ cat: "Trim & finish carpentry", item: "Baseboard", cost: perimeter * r.baseboardPerLinFt, note: "Materials included.", materialQty: { type: "Baseboard (7 ft sticks)", unit: "pcs", qty: Math.ceil(perimeter / 7) } });
  }
  ...
  const floorRate =
    a.flooring === "Tile" ? r.floorTileBasementPerSqft : a.flooring === "Epoxy" ? r.floorEpoxyStandardPerSqft : a.flooring === "Laminate" ? r.floorLaminatePerSqft : r.floorLVPPerSqft;
  if (floorRate != null) {
    const isTile = a.flooring === "Tile";
    const isEpoxy = a.flooring === "Epoxy";
    const labelSuffix = isEpoxy ? " — Labour & Supply" : " — Installation";
    const flooringLabel = isEpoxy ? "Epoxy (basic)" : (a.flooring || "Vinyl (LVP)");
    items.push({
      cat: "Flooring",
      item: `Flooring (${flooringLabel})${labelSuffix}`,
      cost: sqft * floorRate,
  ...
  if (r.floorProtectionPerSqft != null) {
    items.push({ cat: "Prep", item: "Floor protection", cost: sqft * r.floorProtectionPerSqft, note: "Ram Board or equivalent — laid down at the start, protects finished floors for the duration of the job." });
  }
...
function computeDeckFence(a, rates) {
  const list = rates.deckfence;
  const r = rateMap(list);
  const items = [];
  if (a.dfScope === "Fence") {
    ...
    if (a.fenceRepairNeeded === "Yes" && r.fenceRepairFlat != null) {
      items.push({ cat: "Fence", item: "Fence repair (panel/board section) — Labour & Supply", cost: r.fenceRepairFlat, note: "A small separate repair elsewhere along the run." });
    }
  } else if (a.dfScope === "Deck") {
  ...
  items.push(...customItems("deckfence", list));
  return { items, flags: [] };
}
...
function computeInterlocking(a, rates) {
  ...
  if (a.ilMaterial === "Poured concrete (walkway/steps)") {
    if (a.oldConcreteRemoval === "Yes" && r.oldConcreteRemovalPerSqft != null) {
      items.push({ cat: "Concrete", item: "Old concrete/asphalt removal — Labour & Supply", cost: sqft * r.oldConcreteRemovalPerSqft, note: "Break out and haul away before the new pour." });
    }
    if (r.concreteWalkwayPerSqft != null) {
      items.push({ cat: "Concrete", item: "Poured concrete walkway — Labour & Supply", cost: sqft * r.concreteWalkwayPerSqft, note: "Broom finish, includes base prep and forming." });
    }
    const stepsCount = a.stepsCount ?? 0;
    if (stepsCount > 0 && r.concreteStepEach != null) {
      items.push({ cat: "Concrete", item: `Concrete steps (${stepsCount}) — Labour & Supply`, cost: stepsCount * r.concreteStepEach, note: "Framing, rebar/mesh, and pour per step." });
      if (a.stepsTile === "Yes" && r.concreteStepTileEach != null) {
        items.push({ cat: "Concrete", item: `Tile over concrete steps (${stepsCount}) — Labour & Supply`, cost: stepsCount * r.concreteStepTileEach, note: "Tile and edge work per step, material included." });
      }
    }
  ...
  if (a.ilMaterial === "Outdoor porcelain tile") {
    if (r.porcelainTileOutdoorPerSqft != null) {
      items.push({ cat: "Interlocking", item: "Outdoor porcelain tile — Labour & Supply", cost: sqft * r.porcelainTileOutdoorPerSqft, note: "Frost-rated tile, base prep and install included." });
    }
    items.push(...customItems("interlocking", list));
    return { items, flags: [] };
  }
...
function computeFlood(a, rates) {
  ...
  let sqft, perimeter;
  const flags = [];
  if (a.floodLayoutMethod === "Approximate (I know room count + overall size, not exact rooms)") {
    let bLength, bWidth;
    if (a.floodApproxSizeKnown === "Just total sq ft") {
      const approxSqft = a.floodApproxSqft ?? 900;
      const side = Math.sqrt(approxSqft);
      bLength = side;
      bWidth = side;
    } else {
      bLength = a.floodApproxLength ?? 35;
      bWidth = a.floodApproxWidth ?? 25;
    }
    sqft = bLength * bWidth;
    const exteriorPerimeter = 2 * (bLength + bWidth);
    const longerSide = Math.max(bLength, bWidth);
    const shorterSide = Math.min(bLength, bWidth);
    const roomCount = a.floodApproxRoomCount ?? 3;
    const rows = roomCount >= 4 ? 2 : 1;
    const columns = Math.ceil(roomCount / rows);
    const shortPartitions = Math.max(0, columns - 1);
    const longPartitions = Math.max(0, rows - 1);
    const addedInteriorPerimeter = shortPartitions * shorterSide * 2 + longPartitions * longerSide * 2;
    perimeter = exteriorPerimeter + addedInteriorPerimeter;
    flags.push(`Perimeter estimated from room count, not exact measurements: ${perimeter.toFixed(0)} linear ft. Enter exact room dimensions for a firmer number.`);
  } else {
    const roomCount = a.roomCount ?? 1;
    const roomDims = [];
    for (let i = 1; i <= roomCount; i++) {
    ...
    sqft = roomDims.reduce((s, d) => s + d.length * d.width, 0);
    perimeter = roomDims.reduce((s, d) => s + 2 * (d.length + d.width), 0);
  }
  ...
  if (a.builtInFurnitureDamaged === "Yes") {
    if (a.furnitureCondition === "Repairable") {
      ...
      if (r.floodCabinetDemoPerCabinet != null) {
        items.push({ cat: "Demolition & prep", item: `Cabinet demo/removal (${cabinetCount})`, cost: cabinetCount * r.floodCabinetDemoPerCabinet, note: "Removed before repair — cabinets taken off the wall/base, set aside." });
      }
      const kitchenRates = rateMap(rates.kitchen || []);
      if (kitchenRates.cabinetInstallEach != null) {
        items.push({ cat: "Demolition & prep", item: `Cabinet repair & reinstall (${cabinetCount}) — Installation`, cost: cabinetCount * kitchenRates.cabinetInstallEach, note: "Damaged parts repaired, reinstalled. Same rate as Kitchen cabinet install." });
      }
    } else if (r.floodFurnitureHaulAwayPerCabinet != null) {
      const cabinetCount = a.cabinetCount ?? 10;
      items.push({ cat: "Demolition & prep", item: `Built-in furniture — total loss, haul away (${cabinetCount})`, cost: cabinetCount * r.floodFurnitureHaulAwayPerCabinet, note: "Not salvageable — demo and disposal, per cabinet/unit." });
    }
  }
  ...
  if (a.moldVisible === "Yes" && r.moldCleaningFlat != null) {
    items.push({ cat: "Mitigation", item: "Mold cleaning treatment", cost: r.moldCleaningFlat, note: "Surface cleaning/treatment for visible mold. Extensive mold remediation is a specialist job — refer out, not something we price here." });
  }
  ...
  {
    const isFireRatedWall = a.floodWallDrywallThickness === "5/8\" (fire-rated)";
    const wallDrywallRate = isFireRatedWall ? r.floodDrywallRebuildFireRatedPerSqft : r.floodDrywallRebuildPerSqft;
    if (wallDrywallRate != null) {
      items.push({ cat: "Rebuild", item: `New drywall, ${isFireRatedWall ? "5/8\"" : "1/2\""} + taping (${cutHeight} ft band) — Labour & Supply`, cost: cutBandSqft * wallDrywallRate, note: `Board, screws, taping, and mudding for the cut band, ready for paint. ~${drywallSheets(perimeter, cutHeight)} sheets (4×8 ft).`, sheets: { type: isFireRatedWall ? "5/8\" (flood rebuild band)" : "1/2\" (flood rebuild band)", count: drywallSheets(perimeter, cutHeight) } });
    }
  }
  if (a.floodHasCeiling === "Yes") {
    const isFireRatedCeiling = a.floodCeilingDrywallThickness === "5/8\" (fire-rated)";
    const ceilingDrywallRate = isFireRatedCeiling ? r.floodCeilingDrywallFireRatedPerSqft : r.floodCeilingDrywallPerSqft;
    if (ceilingDrywallRate != null) {
      items.push({ cat: "Rebuild", item: `Ceiling drywall, ${isFireRatedCeiling ? "5/8\"" : "1/2\""} + taping — Labour & Supply`, cost: sqft * ceilingDrywallRate, note: `${sqft.toFixed(0)} sq ft. ~${Math.ceil(sqft / 32)} sheets (4×8 ft).`, sheets: { type: isFireRatedCeiling ? "5/8\" (flood ceiling)" : "1/2\" (flood ceiling)", count: Math.ceil(sqft / 32) } });
    }
  }
  ...
  if (a.floodFlooringNeeded === "Yes — tile" && r.floodFlooringTilePerSqft != null) {
    items.push({ cat: "Rebuild", item: "New flooring, tile — Installation", cost: sqft * r.floodFlooringTilePerSqft, note: `${sqft.toFixed(0)} sq ft — concrete slab, no underlayment needed. Tile itself priced separately.`, buys: { name: "Floor tile", low: Math.round(sqft * 3), high: Math.round(sqft * 4) } });
  }
  if (a.floodFlooringNeeded === "Yes — epoxy (basic)" && r.floodFlooringEpoxyPerSqft != null) {
    items.push({ cat: "Rebuild", item: "New flooring, epoxy (basic) — Labour & Supply", cost: sqft * r.floodFlooringEpoxyPerSqft, note: `${sqft.toFixed(0)} sq ft — grind, crack repair, moisture test, base coat, flake, topcoat. For premium finishes or moisture primer, use the Epoxy Floor button for a detailed quote.` });
  }
  ...
  if (a.floodHasBathroom === "Yes") {
    const bathAnswers = {};
    Object.keys(a).forEach((k) => {
      if (k.startsWith("floodBath_")) bathAnswers[k.slice("floodBath_".length)] = a[k];
    });
    const bathResult = computeBathroom(bathAnswers, rates);
  ...
  if (a.contentManipulation === "Yes" && r.contentManipulationFlat != null) {
    items.push({ cat: "Prep", item: "Content manipulation — Labour", cost: r.contentManipulationFlat, note: "Moving furniture and belongings out of the way to access the work area, and back afterward." });
  }
...
function computeEpoxyFloor(a, rates) {
  const list = rates.epoxyfloor || [];
  const r = rateMap(list);
  const length = a.efLength ?? 20;
  const width = a.efWidth ?? 20;
  const sqft = length * width;
  const items = [];
  const flags = [];

  if (a.efCrackSeverity === "Moderate — needs routing/sealing") {
    const linearFt = a.efCrackLinearFt ?? 10;
    if (r.efCrackRepairPerLinFt != null) {
      items.push({ cat: "Prep", item: `Crack repair, moderate (${linearFt} linear ft) — Labour & Supply`, cost: linearFt * r.efCrackRepairPerLinFt, note: "Routing and sealing before grinding/coating — done so the crack doesn't telegraph through the finish." });
    }
  } else if (a.efCrackSeverity === "Major or structural — needs inspection") {
    flags.push("Major/structural cracking noted — this needs a proper inspection and a separate quote before any coating work starts. Not priced in this estimate.");
  } else if (a.efCrackSeverity === "Active water intrusion") {
    flags.push("Active water intrusion noted — this needs a separate waterproofing repair before any coating work starts. Not priced in this estimate.");
  }
  if (a.efCoatingRemoval === "Yes" && r.efCoatingRemovalPerSqft != null) {
    items.push({ cat: "Prep", item: "Old paint/coating removal — Labour & Supply", cost: sqft * r.efCoatingRemovalPerSqft, note: "Grinding off whatever's on there now before the new system goes down." });
  }
  if (a.efAdhesiveRemoval === "Yes" && r.efAdhesiveRemovalPerSqft != null) {
    items.push({ cat: "Prep", item: "Old adhesive/glue removal — Labour & Supply", cost: sqft * r.efAdhesiveRemovalPerSqft, note: "Old tile or carpet glue residue ground off the slab." });
  }
  if (a.efSelfLevel === "Yes" && r.efSelfLevelPerSqft != null) {
    items.push({ cat: "Prep", item: "Self-leveling compound — Labour & Supply", cost: sqft * r.efSelfLevelPerSqft, note: "Evens out a rough or uneven slab before coating." });
  }

  const tierRate =
    a.efTier === "Premium (concrete repair, metallic/premium flake, multi-coat)" ? r.efPremiumPerSqft :
    a.efTier === "Decorative/metallic (full custom colour, showroom finish)" ? r.efDecorativePerSqft :
    r.efBasicPerSqft;
  const tierLabel =
    a.efTier === "Premium (concrete repair, metallic/premium flake, multi-coat)" ? "Premium" :
    a.efTier === "Decorative/metallic (full custom colour, showroom finish)" ? "Decorative/metallic" :
    "Basic";
  if (tierRate != null) {
    items.push({ cat: "Coating", item: `Epoxy floor coating (${tierLabel}) — Labour & Supply`, cost: sqft * tierRate, note: "Diamond grinding, moisture test, epoxy base, flake/finish, and topcoat all included at this tier." });
  }
  if (a.efSpaceType === "Garage" && r.efGaragePremiumFlat != null) {
    items.push({ cat: "Coating", item: "Garage-scale surcharge — Labour & Supply", cost: r.efGaragePremiumFlat, note: "Vehicle traffic and oil-resistance additive for a garage floor." });
  }
  if (a.efMoistureElevated === "Yes — needs vapour-barrier primer" && r.efMoisturePrimerPerSqft != null) {
    items.push({ cat: "Coating", item: "Vapour-barrier primer (elevated moisture) — Labour & Supply", cost: sqft * r.efMoisturePrimerPerSqft, note: "Keeps ground moisture from reaching the epoxy and causing it to lift or bubble later." });
  }

  items.push(...customItems("epoxyfloor", list));
  return { items, flags };
}
...
const COMPUTE = {
  ...
  roof: computeRoof,
  paint: computePaint,
  deckfence: computeDeckFence,
  interlocking: computeInterlocking,
  glass: computeGlass,
  flood: computeFlood,
  epoxyfloor: computeEpoxyFloor,
};
...
const CONTRACTOR_INDEX_KEY = "C-index";
async function getContractorIndex() {
  try {
    const r = await robustStorageGet(CONTRACTOR_INDEX_KEY);
    if (r) return JSON.parse(r.value);
  } catch (err) {}
  return [];
}
async function addToContractorIndex(id) {
  try {
    const idx = await getContractorIndex();
    if (!idx.includes(id)) {
      idx.push(id);
      await robustStorageSet(CONTRACTOR_INDEX_KEY, JSON.stringify(idx));
    }
  } catch (err) {}
}
...
export default function NiagaraEstimatorSite() {
  ...
  const [removedItemIdx, setRemovedItemIdx] = useState(() => new Set());
  const [itemCostOverride, setItemCostOverride] = useState({});
  ...
  const [contractorSaveError, setContractorSaveError] = useState(null);
  ...
  const quickResults = useMemo(() => {
    ...
    function matchScore(labelWords) {
      // Lower is better. Score each query word against its best-matching
      // label word, then sum — so items where every word matches tightly
      // (exact, then prefix, then substring) rank above loose fuzzy hits,
      // and matches for the same term cluster together instead of being
      // scattered in room order.
      let total = 0;
      for (const qw of queryWords) {
        let best = 4; // fallback fuzzy match
        for (const lw of labelWords) {
          const synonyms = SEARCH_SYNONYMS[qw] || [qw];
          for (const syn of synonyms) {
            if (!fuzzyWordMatch(syn, lw)) continue;
            let s;
            if (syn === lw) s = 0;
            else if (lw.startsWith(syn)) s = 1;
            else if (syn.startsWith(lw)) s = 2;
            else s = 3;
            if (s < best) best = s;
          }
        }
    ...
    return allJobs
      ...
      .map((j) => {
        ...
        const matches = queryWords.every((qw) => wordMatchesAny(qw, labelWords));
        return matches ? { job: j, score: matchScore(labelWords) } : null;
      })
      .filter(Boolean)
  ...
  const result = useMemo(() => {
    ...
    const adjustedAll = items.map((it, idx) => ({
      ...it,
      idx,
      cost: itemCostOverride[idx] != null ? itemCostOverride[idx] : it.cost * muni.mult,
    }));
    const adjusted = adjustedAll.filter((it) => !removedItemIdx.has(it.idx));
    const byCategory = {};
    adjusted.forEach((it) => {
    ...
    const sheetsNeeded = Object.entries(sheetsMap).map(([type, count]) => ({ type: `${type} (incl. 10% waste)`, count: Math.ceil(count * 1.1) }));
    ...
    const materialsQtyList = Object.entries(materialQtyMap).map(([key, qty]) => {
      ...
      const isAreaMaterial = unit === "sq ft" && /tile|vinyl|laminate/i.test(type);
      const withBuffer = isAreaMaterial ? qty * 1.1 : qty;
      return { type: isAreaMaterial ? `${type} (incl. 10% waste)` : type, unit, qty: Math.ceil(withBuffer * 100) / 100 };
    });
  ...
  function goNext() {
    if (flowIndex + 1 >= steps.length) {
      ...
      // right after the global municipality question, show a quick "here's how long this takes" screen
      setFlowIndex(flowIndex + 1);
      setPhase("get_started");
    } else {
      setFlowIndex(flowIndex + 1);
    }
  }
  ...
  async function shareEstimateNative(items, subtotal, hst, total, label) {
    const text = buildEstimateText(items, subtotal, hst, total, label);
    if (navigator.share) {
      try {
        await navigator.share({ title: `${label} — estimate`, text });
        return;
      } catch (err) {
        // user cancelled the share sheet, or it failed — fall through to the text view
      }
    }
    showEstimateAsText(items, subtotal, hst, total, label);
  }
  ...
  async function saveLead(fromVisitRequest = false) {
    if (!result) return;
    setLeadSaving(true);
    const id = `L-${Date.now()}`;
    ...
    try {
      ...
      setLeadSaveError(null);
      if (fromVisitRequest) setVisitRequested(true);
    } catch (err) {
      // Don't block the person from seeing their own estimate just because
      // the background save/notification failed — show a soft warning
      // instead, and still let them through.
      setLeadSaveError(`Heads up — this wasn't saved on our end (${err && err.message ? err.message : "unknown error"}). Your estimate still works below; screenshot or download the PDF to keep a copy.`);
    } finally {
      setLeadSent(true);
      setLeadSaving(false);
    }
  ...
  const CONTRACTOR_SPECIALTIES = ["Tile", "Electrical", "Plumbing", "HVAC", "Drywall", "Flooring", "Painting", "Roofing", "Concrete/Interlocking", "Glass", "General/Other"];

  function toggleContractorSpecialty(s) {
    setContractorSpecialties((prev) => (prev.includes(s) ? prev.filter((x) => x !== s) : [...prev, s]));
  }
  ...
  async function saveContractor() {
    ...
    try {
      await robustStorageSet(id, JSON.stringify(record));
      await addToContractorIndex(id);
      setContractorSaveError(null);
    } catch (err) {
      setContractorSaveError(`Heads up — this wasn't saved on our end (${err && err.message ? err.message : "unknown error"}).`);
    } finally {
  ...
  async function loadContractors() {
    ...
    try {
      ...
      for (const id of idx) {
        try {
          const r = await robustStorageGet(id);
  ...
  return (
    ...
    >
      ...
      {/* Nav */}
      <div className="max-w-4xl mx-auto px-6 pt-3 flex items-center justify-center">
        ...
        {phase !== "landing" && (
          <span className="text-xs uppercase tracking-wide" style={{ color: "#64748B" }}>
            ...
            {phase === "get_started" && "Almost there"}
            {phase === "chat" && "Chatting"}
      ...
      {/* progress bar */}
      {(phase === "flow" || phase === "get_started" || phase === "chat") && (
      ...
      <div className="max-w-4xl mx-auto px-6 pb-24">
        ...
        {phase === "landing" && (
          <div>
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      ...
                      style={{ backgroundColor: "#FDE047", color: "#451A03" }}
                    >
                      {estimateTimeLabel(t.id)}
                    </span>
                    <div className="relative h-full flex flex-col justify-end p-3">
            ...
            <div className="text-center mt-6">
              <button
                onClick={() => { setPhase("contractor_signup"); setContractorSaved(false); }}
        ...
        {/* ---------------- CONTRACTOR SIGNUP ---------------- */}
        {phase === "contractor_signup" && (
          <div className="pt-8">
            <button onClick={() => setPhase("landing")} className="inline-flex items-center gap-1 text-sm mb-4" style={{ color: "#475569" }}>
              <ChevronLeft size={16} /> Back
            </button>
            {!contractorSaved ? (
              <div className="bg-white rounded-xl border shadow-lg p-6" style={{ borderColor: "#DCEAFB" }}>
                <p className="text-lg font-semibold">Join our contractor network</p>
                <p className="text-sm mt-1" style={{ color: "#475569" }}>
                  Tell us what you do — when a job comes in that fits your specialty, or we need an extra hand, we'll know who to call.
                </p>
                <div className="space-y-3 mt-5">
                  ...
                  />
                  <div>
                    <p className="text-xs font-medium mb-2" style={{ color: "#475569" }}>What do you do? (pick all that apply)</p>
                    <div className="flex flex-wrap gap-2">
                      {CONTRACTOR_SPECIALTIES.map((s) => (
                        <button
                          key={s}
                          type="button"
                          onClick={() => toggleContractorSpecialty(s)}
                          className="text-xs px-3 py-1.5 rounded-full border-2 font-medium transition-colors"
                          style={
                            contractorSpecialties.includes(s)
                              ? { backgroundColor: WINE, borderColor: WINE, color: "white" }
                              : { borderColor: "#DCEAFB", color: "#475569" }
                          }
                        >
                          {s}
                        </button>
                  ...
                  <input
                    type="text"
                    placeholder="Service area (e.g. St. Catharines, Niagara Falls...)"
                    value={contractorArea}
                    onChange={(e) => setContractorArea(e.target.value)}
                    className="w-full border rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2"
                    style={{ borderColor: "#DCEAFB" }}
                  />
                  <textarea
                    placeholder="Anything else? (rates, availability, years of experience...)"
                ...
                {contractorSaveError && (
                  <div className="flex items-start gap-2 mt-3 text-xs" style={{ color: "#B45309" }}>
                    <AlertTriangle size={13} /> {contractorSaveError}
                  </div>
                )}
                <button
                  onClick={saveContractor}
                  disabled={contractorSaving || !contractorName}
                  className="w-full mt-5 inline-flex items-center justify-center gap-2 text-white px-4 py-3 rounded-lg font-medium disabled:opacity-50"
                  style={{ backgroundColor: WINE }}
                >
                  {contractorSaving ? <Loader2 size={16} className="animate-spin" /> : null}
                  Join the network
                </button>
              </div>
            ) : (
              <div className="bg-white rounded-xl border shadow-lg p-6 text-center" style={{ borderColor: "#DCEAFB" }}>
                <CheckCircle2 size={32} style={{ color: GREEN }} className="mx-auto mb-2" />
                <p className="font-semibold">You're on the list!</p>
                <p className="text-sm mt-1" style={{ color: "#64748B" }}>We'll reach out when a job matching your specialty comes up.</p>
                <button onClick={() => setPhase("landing")} className="mt-4 text-sm underline" style={{ color: WINE }}>
                  Back to home
                </button>
              </div>
            )}
        ...
        {/* ---------------- CONTRACTOR DIRECTORY (admin view) ---------------- */}
        {phase === "contractor_directory" && (
          <div className="pt-8">
            <button onClick={() => setPhase("landing")} className="inline-flex items-center gap-1 text-sm mb-4" style={{ color: "#475569" }}>
              <ChevronLeft size={16} /> Back
            </button>
            <p className="text-lg font-semibold mb-3">Contractor network</p>
            <input
              type="text"
              placeholder="Filter by specialty (e.g. Tile)"
              value={contractorFilter}
              onChange={(e) => setContractorFilter(e.target.value)}
              className="w-full border rounded-lg px-4 py-2.5 text-sm mb-4 focus:outline-none focus:ring-2"
              style={{ borderColor: "#DCEAFB" }}
            />
            {contractors.filter((c) => !contractorFilter || c.specialties.some((s) => s.toLowerCase().includes(contractorFilter.toLowerCase()))).length === 0 && (
              <p className="text-sm" style={{ color: "#94A3B8" }}>No contractors match yet.</p>
            )}
            <div className="space-y-3">
              {contractors
                .filter((c) => !contractorFilter || c.specialties.some((s) => s.toLowerCase().includes(contractorFilter.toLowerCase())))
                .map((c) => (
                  <div key={c.id} className="bg-white rounded-xl border shadow-lg p-4" style={{ borderColor: "#DCEAFB" }}>
                    <p className="font-medium">{c.name} {c.company && <span style={{ color: "#94A3B8" }}>— {c.company}</span>}</p>
                    <p className="text-sm mt-1" style={{ color: "#475569" }}>{c.phone} {c.email && `· ${c.email}`}</p>
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {c.specialties.map((s) => (
                        <span key={s} className="text-xs px-2 py-0.5 rounded-full" style={{ backgroundColor: "#EFF6FF", color: WINE }}>{s}</span>
                      ))}
                    </div>
                    {c.area && <p className="text-xs mt-2" style={{ color: "#94A3B8" }}>Area: {c.area}</p>}
                    {c.notes && <p className="text-xs mt-1" style={{ color: "#94A3B8" }}>{c.notes}</p>}
                  </div>
                ))}
            </div>
          </div>
        ...
        {phase === "quickjob" && (
          <div className="pt-8">
            {quickCart.length > 0 && (
              <div className="fixed right-4 z-40 flex items-center gap-2 no-print" style={{ top: "calc(env(safe-area-inset-top, 0px) + 16px)" }}>
                <button
                  onClick={() => window.print()}
                  className="inline-flex items-center gap-1.5 text-white pl-3 pr-3.5 py-2 rounded-full shadow-lg text-xs font-semibold"
                  style={{ backgroundColor: WINE }}
                  title="Download / print this estimate as PDF"
                >
                ...
                {!estimateSaved && (
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 pl-3 pr-3.5 py-2 rounded-full shadow-lg text-xs font-semibold bg-white border-2"
                    style={{ borderColor: WINE, color: WINE }}
                    title="Save this estimate in-app"
                  >
                    <Save size={14} /> Save
                  </button>
                )}
            ...
            <button onClick={() => setPhase("landing")} className="inline-flex items-center gap-1 text-sm mb-4 no-print" style={{ color: "#475569" }}>
              <ChevronLeft size={16} /> Back
            </button>
            ...
            <div className="mt-5 flex items-center gap-2 bg-white rounded-lg border shadow-lg px-3 py-2.5 no-print" style={{ borderColor: "#DCEAFB" }}>
            ...
            {quickCart.length > 0 && (
              <>
                <button
                  onClick={() => window.print()}
                  className="mt-4 w-full inline-flex items-center justify-center gap-2 text-white px-4 py-3 rounded-lg font-medium no-print"
                  style={{ backgroundColor: WINE }}
                >
                  <Download size={16} /> Download / print this estimate as PDF
                </button>
                <p className="text-xs text-center mt-1.5 no-print" style={{ color: "#64748B" }}>Opens your browser's print dialog — choose "Save as PDF," or print it, or share it straight from there. Works every time, on any device.</p>
              </>
            )}
              ...
              <div className="mt-4 bg-white rounded-xl border shadow-lg p-4" style={{ borderColor: "#DCEAFB" }}>
                {!showSaveEstimate ? (
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs" style={{ color: "#94A3B8" }}>No need to use your real name or address if you'd rather not — any nickname works, as long as it's enough to find this estimate again.</p>
                    <input
                      type="text"
                      placeholder="Client name (or a nickname)"
                    ...
                    <input
                      type="text"
                      placeholder="Address (or a general area)"
                      value={saveAddress}
            ...
            {quickShowLead && !quickLeadSent && (
              <div className="mt-6 bg-white border rounded-xl p-5 shadow-lg space-y-2" style={{ borderColor: "#DCEAFB" }}>
                ...
                <p className="text-xs mb-2" style={{ color: "#64748B" }}>No need to use your real name or address if you'd rather not — any nickname works, as long as we can match it back to your estimate later.</p>
                <input
                  type="text"
                ...
                <input
                  type="text"
                  placeholder="Project address (or a general area)"
                  value={leadAddress}
        ...
        {phase === "get_started" && (
          <div className="pt-8">
            <p className="text-sm" style={{ color: "#475569" }}>
              Just a few questions left — nothing complicated, mostly taps.
            </p>
            <div className="mt-3 inline-flex items-center gap-2 px-4 py-2 rounded-full" style={{ backgroundColor: "#FDE047" }}>
              <CalendarDays size={16} style={{ color: "#451A03" }} />
              <span className="text-sm font-bold" style={{ color: "#451A03" }}>
                {estimateTimeLabel(roomType)} to answer
              </span>
            </div>
            <button
              onClick={() => setPhase("flow")}
              className="mt-6 w-full text-white px-6 py-3.5 rounded-lg font-semibold text-base"
              style={{ backgroundColor: WINE }}
            >
              Get started
            </button>
                  ...
                  {q.type === "number" && (
                    <div className="flex items-center gap-3">
                      <button
                        type="button"
                        onClick={() => {
                          const current = answers[q.id] ?? q.default;
                          const next = Math.max(q.min, +(current - q.step).toFixed(2));
                          setAnswers({ ...answers, [q.id]: next });
                        }}
                        className="shrink-0 w-9 h-9 rounded-full border-2 flex items-center justify-center text-lg font-bold active:scale-95 transition-transform"
                        style={{ borderColor: "#DCEAFB", color: WINE }}
                        aria-label="Decrease"
                      >
                        −
                      ...
                      <button
                        ...
                        onClick={() => {
                          const current = answers[q.id] ?? q.default;
                          const next = Math.min(q.max, +(current + q.step).toFixed(2));
                          setAnswers({ ...answers, [q.id]: next });
                        }}
                        className="shrink-0 w-9 h-9 rounded-full border-2 flex items-center justify-center text-lg font-bold active:scale-95 transition-transform"
                        style={{ borderColor: "#DCEAFB", color: WINE }}
                      ...
                      <span className="text-2xl w-24 text-right shrink-0" style={{ fontFamily: DISPLAY_FONT, fontWeight: 800 }}>
        ...
        {phase === "result" && result && !leadSent && (
          <div className="pt-8">
            ...
            <div className="bg-white rounded-xl border shadow-lg p-6 sm:p-8" style={{ borderColor: "#DCEAFB" }}>
              <p className="text-sm font-semibold">Your estimate is ready</p>
              <p className="text-sm mt-1" style={{ color: "#475569" }}>
                One quick step first — where should we send it? No need for your real name or address if you'd rather not; any nickname works, as long as we can match it back to you.
              </p>
              ...
              <p className="text-xs mt-2" style={{ color: "#94A3B8" }}>
                Municipality: {result.muni.label} — included automatically.
              </p>
              <button
                onClick={() => saveLead(false)}
                disabled={leadSaving}
                className="w-full mt-4 inline-flex items-center justify-center gap-2 text-white px-4 py-3 rounded-lg font-medium disabled:opacity-60"
                style={{ backgroundColor: WINE }}
              >
                {leadSaving ? <Loader2 size={16} className="animate-spin" /> : null}
                See my estimate
              </button>
            </div>
          </div>
        ...
        {phase === "result" && result && leadSent && (
          <div className="pt-8">
            ...
            <div className="bg-white rounded-xl border shadow-lg p-6 sm:p-8 relative overflow-hidden" style={{ borderColor: "#DCEAFB" }}>
              ...
              <div className="mt-4 pt-4 border-t no-print" style={{ borderColor: "#E6F0FC" }}>
                <button
                  onClick={() => setShowEditMeasurements((v) => !v)}
                  className="inline-flex items-center gap-1.5 text-sm font-medium"
                  style={{ color: WINE }}
                >
                  <Settings size={14} /> {showEditMeasurements ? "Hide" : "Edit measurements"}
                </button>
                {showEditMeasurements && (
                  <div className="mt-3 space-y-4">
                    {questions.filter((q) => q.type === "number").length === 0 && (
                      <p className="text-xs" style={{ color: "#94A3B8" }}>Nothing numeric to adjust here — use "Edit answers" below for multiple-choice questions.</p>
                    )}
                    {questions.filter((q) => q.type === "number").map((q) => (
                      <div key={q.id}>
                        <p className="text-xs mb-1.5" style={{ color: "#475569" }}>{q.label}</p>
                        <div className="flex items-center gap-2">
                          <button
                            type="button"
                            onClick={() => {
                            ...
                            className="shrink-0 w-8 h-8 rounded-full border-2 flex items-center justify-center text-base font-bold active:scale-95 transition-transform"
                            style={{ borderColor: "#DCEAFB", color: WINE }}
                            aria-label="Decrease"
                          >
                            −
                          ...
                          <span className="text-sm w-20 text-right shrink-0 font-medium">
                            {answers[q.id] ?? q.default} <span style={{ color: "#94A3B8" }}>{q.unit}</span>
                          </span>
                        </div>
                      </div>
                    ))}
            ...
            {/* PDF / print — the reliable way to keep an estimate, doesn't depend on browser storage */}
            <button
              onClick={() => window.print()}
              className="mt-4 w-full inline-flex items-center justify-center gap-2 text-white px-4 py-3 rounded-lg font-medium no-print"
              style={{ backgroundColor: WINE }}
            ...
            {typeof navigator !== "undefined" && navigator.share && (
              <button
                onClick={() => shareEstimateNative(result.items, result.subtotal, result.hst, result.total, ROOM_TYPES.find((rt) => rt.id === roomType)?.label || "Estimate")}
                className="mt-2 w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg font-medium border-2 transition-colors hover:bg-[#EFF6FF] no-print"
                style={{ borderColor: "#DCEAFB", color: WINE }}
              >
                <Send size={15} /> Share (Messages, Email, Notes…)
              </button>
            )}

            {/* Save estimate — for the admin's own record-keeping, separate from client lead capture */}
            {!estimateSaved ? (
              <div className="mt-3 bg-white rounded-xl border shadow-lg p-4 no-print" style={{ borderColor: "#DCEAFB" }}>
                {!showSaveEstimate ? (
                  <button
                    onClick={() => setShowSaveEstimate(true)}
                    className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg font-medium border-2 transition-colors hover:bg-[#EFF6FF]"
                    style={{ borderColor: "#DCEAFB", color: WINE }}
                  >
                    <Save size={16} /> Also save in-app (this browser only)
                  </button>
                ) : (
                  <div className="space-y-2">
                    <p className="text-sm font-medium">Save this estimate</p>
                    <p className="text-xs" style={{ color: "#94A3B8" }}>Stored on this device/browser only — it won't show up if you switch phones or browsers. The PDF above is the dependable copy. No need to use your real name or address if you'd rather not — any nickname works, as long as it's enough to find this estimate again.</p>
                    <input
                      type="text"
                      placeholder="Client name (or a nickname)"
                      value={saveClientName}
            ...
            {paid ? (
              <>
                ...
                <div className="mt-8 pt-2 text-center">
                  <p className="text-2xl font-bold tracking-tight" style={{ color: WINE, fontFamily: DISPLAY_FONT }}>Full Estimate</p>
                  <div className="mt-2 mx-auto" style={{ width: 56, height: 3, backgroundColor: WINE, borderRadius: 2 }} />
                </div>
                {/* line item table */}
                <div className="mt-6 bg-white rounded-xl border shadow-lg overflow-hidden" style={{ borderColor: "#DCEAFB" }}>
                  ...
                  <table className="w-full text-sm mt-3">
                    <tbody>
                      {result.items.filter((it) => !it.hideFromBreakdown).map((it, i) => (
                        <tr key={it.idx} className="border-t align-top" style={{ borderColor: "#E6F0FC" }}>
                          <td className="py-2 px-5">
                            <p className="text-base font-medium" style={{ color: INK }}>{it.item}</p>
                            {it.note && <p className="text-xs mt-0.5" style={{ color: "#64748B" }}>{it.note}</p>}
                          </td>
                          <td className="py-2 px-5 text-right font-medium whitespace-nowrap no-print">
                            <input
                              type="number"
                              step="1"
                              value={Math.round(it.cost)}
                              onChange={(e) => {
                                const v = e.target.value === "" ? 0 : Number(e.target.value);
                                setItemCostOverride((prev) => ({ ...prev, [it.idx]: v }));
                              }}
                              className="w-24 border rounded-lg px-2 py-1 text-right text-base font-semibold focus:outline-none focus:ring-2"
                              style={{ borderColor: "#DCEAFB" }}
                            />
                          </td>
                          <td className="py-2 pl-2 pr-5 text-right whitespace-nowrap print-only text-base font-semibold">
                            {formatCAD(it.cost)}
                          </td>
                          <td className="py-2 pl-1 pr-4 no-print">
                            {confirmRemoveIdx === it.idx ? (
                              <div className="flex items-center gap-1">
                                <button
                                  onClick={() => { setRemovedItemIdx((prev) => new Set(prev).add(it.idx)); setConfirmRemoveIdx(null); }}
                              ...
                              <button
                                onClick={() => setConfirmRemoveIdx(it.idx)}
                                className="p-1.5 rounded-lg hover:bg-[#FFF7ED]"
                                style={{ color: AMBER }}
                  ...
                  <p className="text-xs px-5 pb-3 no-print" style={{ color: "#94A3B8" }}>Tap a price to edit it, or the × to remove a line — totals update right away.</p>
                ...
                <p className="text-xs text-center mt-6 no-print" style={{ color: "#94A3B8" }}>Need a copy? Use "Download / print this estimate as PDF" near the top.</p>

                <div className="mt-8 pt-6 border-t no-print" style={{ borderColor: "#E6F0FC" }}>
                  {!estimateSaved ? (
                    <button
                      onClick={() => setShowSaveEstimate(true)}
                      className="w-full inline-flex items-center justify-center gap-2 px-4 py-3 rounded-lg font-medium border-2"
                      style={{ borderColor: WINE, color: WINE }}
                    >
                      <Save size={16} /> Save this full estimate
                    </button>
                  ) : (
                    <div className="flex items-center justify-center gap-2 text-sm py-3" style={{ color: GREEN }}>
                      <CheckCircle2 size={16} /> Saved
                    </div>
                  )}
                  <button
                    onClick={resetAll}
                    className="w-full mt-3 inline-flex items-center justify-center gap-2 px-4 py-3 rounded-lg font-medium text-white"
                    style={{ backgroundColor: WINE }}
                  >
                    Finish session — start a new estimate
                  </button>
                </div>
              </>
            ) : (
            ...
            {/* ---------------- request an on-site visit (needs email specifically) ---------------- */}
            {!showLead && !visitRequested && (
              <button
                onClick={() => setShowLead(true)}
                className="mt-6 w-full text-white px-6 py-4 rounded-xl font-semibold transition-transform hover:scale-[1.01] flex items-center justify-center gap-3 shadow-lg no-print"
                style={{ background: `linear-gradient(135deg, ${WINE} 0%, #451A03 100%)` }}
              >
                <span className="rounded-full p-2" style={{ backgroundColor: "rgba(255,255,255,0.15)" }}>
                  <Phone size={18} />
                </span>
                <span className="text-left">
                  <span className="block text-base">Talk to a manager</span>
                  <span className="block text-xs font-normal" style={{ color: "rgba(255,255,255,0.8)" }}>Request an on-site visit</span>
                </span>
              </button>
            )}
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4" style={{ backgroundColor: "rgba(15,23,42,0.5)" }}>
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-5" onClick={(e) => e.stopPropagation()}>
            ...
            <textarea
              ...
              rows={14}
              className="w-full border rounded-lg px-3 py-3 text-sm leading-relaxed focus:outline-none focus:ring-2"
              style={{ borderColor: "#DCEAFB", fontFamily: "'Inter', ui-sans-serif, system-ui, sans-serif" }}
            />
            <button
